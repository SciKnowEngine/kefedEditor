...
from kefed_converter import KefedConverter
...
def run_main(args):
    ...
    kc = KefedConverter()
    m = kc.get_model_from_name(args.inputModel)
    df = kc.build_robot_spreadsheet(m)
    df.to_csv(args.outPath, sep="\t", index=False)

    #models = kc.list_all_models()
    #print len(models)

    #entities = kc.list_all_entities()
    #print len(entities)

    #processes = kc.list_all_processes()
    #print len(processes)

    #variables = kc.list_all_variables()
    #print len(variables)
                ...
                if( opN_match is not None and
                    ...
                    for k in xref_namespace_lookup.keys():
                        if( l.startswith(k) ):
                            ...
                            if( xref_vb is None ):
                                print "error in looking up " + s
                                break
                            resolved_xref_vb = vb_lookup.get(xref_vb,None)
                            if( resolved_xref_vb is None ):
                                print "error in resolving " + xref_vb + " in " + cur_id
                                break
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--inputModel', help='name of KEfED Model to be processed in the repository')
